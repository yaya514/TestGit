...
<script type="text/javascript">
	function changePage(currentPage){
		...
		location.href = "${pageContext.request.contextPath }/teacher_listStudent.html?currentPage="+ currentPage + "&searchName="+searchName;
	}
	...
	function searchName(){
		var searchName = $("#searchName").val();
		location.href = "${pageContext.request.contextPath }/teacher_listStudent.html?searchName="+searchName;
	}
	
	function resetTeacher(){
		var v = window.confirm("您确定要重置已选的指导老师吗？");
		if(v){
			location.href = "${pageContext.request.contextPath }/teacher_reset.html";
		}
	}
	
	function chooseTeacher(teacher_number){
		var confirm = window.confirm("您确定要选中该教师作为指导老师吗？");
		if(confirm){
			location.href = "${pageContext.request.contextPath }/teacher_choose.html?teacher_number="+teacher_number;
		}
}
</script>
...
<body>
	<form id="form"
		...
		<div class="x-body" style="height: 1050px; width: 1113px">
			<xblock> 
			<a class="layui-btn" onclick="resetTeacher()"> 
				重置指导老师
			</a>
				...
				<c:forEach items="${page.list }" var="teacher" varStatus="vs">
					<tbody id="x-img${teacher.teacher_number }">
						<tr>
							...
							<c:if test="${personal.teacher_number != teacher.teacher_number}">
								<td class="td-manage">
									<a title="编辑" href="javascript:void(0)"
										onclick="chooseTeacher(${teacher.teacher_number})"> 
										<i class="layui-icon" style="font-size: 15px; color: blue">&#xe654;选择指导老师</i>
									</a>
								</td>
							</c:if>
							<c:if test="${personal.teacher_number == teacher.teacher_number}">
								<td class="td-manage">
									<span title="编辑" href="javascript:;"
										class="ml-5" style="text-decoration: none"> <i
											class="layui-icon" style="font-size: 15px; color: red">已选择</i>
									</span> 
								</td>
							</c:if>
