...
<script type="text/javascript">
	...
	function deleteTopic(id){
		var v = window.confirm("您确定要删除该课题信息吗？");
		if(v){
			location.href = "${pageContext.request.contextPath }/topic_delete.html?id="+id;
		}
	}
	...
	function addTopic(){
		location.href = "${pageContext.request.contextPath }/topic_add.jsp";
	}
	
	function deleteAll(){
		var confirm = window.confirm("您确定要删除选中的课题吗？");
		if(confirm){
			$("#form").submit();
		}
}
...
<body>
	<form id="form"
		...
		<div class="x-body" style="height: 1050px; width: 1113px">
			...
			</a> <a class="layui-btn" onclick="addTopic()"> <i class="layui-icon">&#xe608;</i>添加
			</a>
				...
				<c:forEach items="${page.list }" var="topic" varStatus="vs">
					<tbody id="x-img${topic.topic_id }">
						<tr>
							...
							<td>${topic.content}</td>
							<td>${topic.teacher.teacher_name }</td>
							<td class="td-manage">
								<a title="编辑" href="javascript:;"
									onclick="member_edit('编辑','topic_show.html','${topic.topic_id}','','510')"
									class="ml-5" style="text-decoration: none"> <i
										class="layui-icon" style="font-size: 25px; color: blue">&#xe642;</i>
								</a> 
								<a title="删除" href="javascript:;"
									onclick="deleteTopic(${topic.topic_id})" style="text-decoration: none">
