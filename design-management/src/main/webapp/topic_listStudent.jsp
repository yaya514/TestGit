...
<script type="text/javascript">
	...
	function resetTopic(){
		var confirm = window.confirm("您确定要重置已选课题吗？");
		if(confirm){
			location.href = "${pageContext.request.contextPath }/topic_reset.html";
		}
...
<body>
	<form id="form"
		...
		<div class="x-body" style="height: 1050px; width: 1113px">
			<xblock> 
			<a class="layui-btn" onclick="resetTopic()"> 
				重置选题
			</a>
				...
				<c:forEach items="${page.list }" var="topic" varStatus="vs">
					<tbody id="x-img${topic.topic_id }">
						<tr>
							...
							<c:if test="${personal.topic_id != topic.topic_id}">
								<td class="td-manage">
									<a title="编辑" href="javascript:void(0)"
										onclick="chooseTopic(${topic.topic_id})"> 
										<i class="layui-icon" style="font-size: 15px; color: blue">&#xe654;选择</i>
									</a>
								</td>
							</c:if>
							<c:if test="${personal.topic_id == topic.topic_id}">
